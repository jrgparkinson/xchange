...
def populate():
    ...
    season = Season.objects.get_or_create(
        ...
        end_time=datetime(2020, 9, 30, 23, 59, 59),
    )[0]
    season.save()
    ...
    events = [
        {
            ...
            "date": datetime(year=2020, month=5, day=11, tzinfo=pytz.UTC),
            "races": [],
        },
        {
            "name": "Cuppers",
            "date": datetime(year=2020, month=10, day=20, tzinfo=pytz.UTC),
            "races": [],
        },
        {
            "name": "MK Cross Challenge",
            "date": datetime(year=2020, month=11, day=5, tzinfo=pytz.UTC),
            "races": ["Senior Men", "Senior Women"],
        },
        {
            "name": "Varsity II-IV",
            "date": datetime(year=2020, month=11, day=28, tzinfo=pytz.UTC),
            "races": ["Men II", "Women II", "Men III", "Men IV", "Women III"],
        },
        {
            "name": "Varsity Blues",
            "date": datetime(year=2020, month=12, day=6, tzinfo=pytz.UTC),
            "races": ["Men", "Women"],
        },
    ]
    ...
    for e in events:
        event = Event.objects.get_or_create(
            name=e["name"], date=e["date"], season=season
        )[0]
        event.save()
        ...
        for r in races:
            race = Race(
                ...
                event=event,
                time=event.date,
                max_dividend=20.0 + random.random() * 100.0,
            )
            race.save()
            ...
            for i in range(len(athletes)):
                result = Result(
                    ...
                    race=race,
                    position=i,
                    time=timedelta(minutes=i, seconds=random.random() * 60.0),
                )
                result.save()
    ...
    for share in Share.objects.all():
        ...
        for i in range(len(investors)):
            ...
            try:
                t = Trade.make_share_trade(
                    athlete=share.athlete,
                    volume=r[i] * share.volume,
                    creator=bank,
                    price=random_price(r[i] * share.volume),
                    seller=bank,
                    buyer=investors[i],
                )
    ...
    for investor in Investor.objects.all():
        ...
        for i in range(3):
            ...
            try:
                Trade.make_share_trade(
                    share_to_sell.athlete,
                    volume,
                    creator=investor,
                    price=random_price(volume),
                    seller=investor,
                )
